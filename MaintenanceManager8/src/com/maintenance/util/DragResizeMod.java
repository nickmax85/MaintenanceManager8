...
public class DragResizeMod {
	...
	private final static int snaptoPixel = 6;
	...
	private static final OnDragResizeEventListener defaultListener = new OnDragResizeEventListener() {
		...
		public void onDrag(Node node, double x, double y, double h, double w) {
			...
			if (node.getUserData() instanceof Leerflaeche) {

				Leerflaeche leerflaeche = (Leerflaeche) node.getUserData();

				leerflaeche.getPanelFormat().setX(snapToPixel(x));
				leerflaeche.getPanelFormat().setY(snapToPixel(y));
				leerflaeche.getPanelFormat().setHeigth(snapToPixel(h));
				leerflaeche.getPanelFormat().setWidth(snapToPixel(w));

				Service.getInstance().updatePanelFormat(leerflaeche.getPanelFormat());
				setNodeSize(node, leerflaeche.getPanelFormat().getX(), leerflaeche.getPanelFormat().getY(),
						leerflaeche.getPanelFormat().getHeigth(), leerflaeche.getPanelFormat().getWidth());
			}
			if (node.getUserData() instanceof Anlage) {

				Anlage anlage = (Anlage) node.getUserData();

				anlage.getPanelFormat().setX(snapToPixel(x));
				anlage.getPanelFormat().setY(snapToPixel(y));
				anlage.getPanelFormat().setHeigth(snapToPixel(h));
				anlage.getPanelFormat().setWidth(snapToPixel(w));

				Service.getInstance().updatePanelFormat(anlage.getPanelFormat());
				setNodeSize(node, anlage.getPanelFormat().getX(), anlage.getPanelFormat().getY(),
						anlage.getPanelFormat().getHeigth(), anlage.getPanelFormat().getWidth());
			}

			if (node.getUserData() instanceof Station) {

				Station station = (Station) node.getUserData();

				station.getPanelFormat().setX(snapToPixel(x));
				station.getPanelFormat().setY(snapToPixel(y));
				station.getPanelFormat().setHeigth(snapToPixel(h));
				station.getPanelFormat().setWidth(snapToPixel(w));

				Service.getInstance().updatePanelFormat(station.getPanelFormat());
				setNodeSize(node, station.getPanelFormat().getX(), station.getPanelFormat().getY(),
						station.getPanelFormat().getHeigth(), station.getPanelFormat().getWidth());
			}

		}

		private int snapToPixel(double pixel) {

			int round = (int) Math.round(pixel / snaptoPixel) * snaptoPixel;
		...
		private void setNodeSize(Node node, double x, double y, double h, double w) {
			...
			if (node instanceof Canvas) {
				...
				((Pane) node).setPrefWidth(w);
				((Pane) node).setPrefHeight(h);
			}
		}
	};
	...
	public static void makeResizable(Node node, OnDragResizeEventListener listener) {
		...
		node.setOnMousePressed(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				if (LayoutController.dragResize || StationOverviewController.dragResize)
